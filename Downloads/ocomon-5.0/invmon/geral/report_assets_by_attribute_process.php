...
$asset_attribute = (isset($post['asset_attribute']) ? noHtml($post['asset_attribute']) : "");
...
$criteriaAttribute = (empty($asset_attribute) ? TRANS('ATTRIBUTE') . ':&nbsp;' . TRANS('ALL') : TRANS('ATTRIBUTE') . ':&nbsp;' . getMeasureTypes($conn, $asset_attribute)['mt_name']);
...
if (!empty($asset_attribute)) {
    // $termsTotal .= (!empty($termsTotal) ? " AND " : " WHERE ");
    $termsTotal .= " AND mt.id = '{$asset_attribute}' ";
    $terms .= " AND mt.id = '{$asset_attribute}' ";
}
...
$criteriaText .= $criteriaClient . '<br />' . $criteriaUnits . '<br />' . $criteriaCategory . '<br />' . $criteriaTypes . '<br />' . $criteriaAttribute;
